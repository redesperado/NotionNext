...
export function GlobalContextProvider(props) {
  ...
  function switchTheme() {
    ...
    const query = router.query
    const currentTheme = query.theme || theme
    const MY_THEMES = ['hexo','heo']
    const currentIndex = MY_THEMES.indexOf(currentTheme)
    const newIndex = currentIndex < MY_THEMES.length - 1 ? currentIndex + 1 : 0
    const newTheme = MY_THEMES[newIndex]
    query.theme = newTheme
    router.push({ pathname: router.pathname, query })
    return newTheme
  }
